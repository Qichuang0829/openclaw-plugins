...
`todo-state.json` 是索引文件，保存每个 todo 的摘要。`todo.json` 是单个 todo 的完整状态。
...
这是索引用的轻量摘要。它让 `astronclaw_todo_get` 或 HTTP 列表接口能展示当前会话有哪些 todo、每个 todo 的整体状态、各状态事项数量和关闭时间。
...
这是 `todo-state.json` 的文件结构，用于保存所有 todo 摘要。
...
工具层按 `sessionKey` 隔离。每次工具注册时都会从 OpenClaw tool context 读取当前 `sessionKey`，并写入 todo 状态。

隔离规则：

- `astronclaw_todo_create` 创建的 todo 绑定当前 `sessionKey`。
