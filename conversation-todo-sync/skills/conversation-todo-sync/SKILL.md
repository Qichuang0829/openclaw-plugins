...
description: 在稍复杂、多步骤、准备清单、资料查询、分析比较、方案制定、分阶段执行或跨多轮任务中，创建并维护当前 conversation todo/checklist 状态。用于把 Agent 当前任务进度同步给用户界面或 HTTP 客户端查看；简单问答、翻译、命名、单句解释等不要主动使用。
---
...
当任务满足以下任一条件时，应使用本工具组同步 todo 状态，除非它明显只是简单问答、单个事实查询、翻译、改写、起名或单句解释：

- 需要查询、阅读、比较、归纳多个来源或多类信息。
- 需要围绕多个对象或多个指标搜集资料并整理结论，例如多家公司股价/涨跌/市值/近期消息对比。即使用户只要求一轮完成，也属于应同步 todo 的任务。
- 需要先拆解，再执行，再汇总结论。
- 需要调用搜索、网页读取、接口查询、文件处理等外部工具，并且可以拆成 3 个以上用户可见的实际阶段。
- 需要持续跟踪多个 todo item 的完成情况。
- 用户要求“完整流程”“逐步推进”“跟踪任务”“不要一次性草率完成”。
- 任务可能持续多轮，或用户后续可能询问当前进度。
- 任务要求输出准备清单、时间顺序、携带物品、注意事项、检查流程、课前/出门前/活动前准备，并且包含 3 个以上交付部分。

不要因为任务“逻辑线性”“一轮可以完成”“只是信息查询加整理”而跳过 todo。todo 的用途是把用户可见进度同步给 UI/HTTP 客户端，不是只用于跨轮任务。只要任务包含多个可展示的实际阶段，就创建并维护 todo。
